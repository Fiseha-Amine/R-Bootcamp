...
Am Schluss:
  ...
  - alle "labels" benannt? {r}
        ...
        - Tabelle einbauen? --> Bei den Zielen
        - etwas mehr Text bei den Analysen schreiben
        - Würdest du sagen der Aim 5 passt zu der Analyse die gemacht wurde?
        - Warnungen unterdrücken (warning = FALSE) (?)
...
head(data_clean)
write.csv(data_clean, "data_clean.csv") #for Shiny app
```
...
```{r, MissingValuesPlot, fig.height = 10, fig.width = 17}
...
### 3. Aim: Where are the most expensive apartments

```{r, top10 most expensive}
...
```{r, subset of top10 most expensive}
data_subset <- top10_sorted %>% select(W.Type,Zimmer, W.Fläche, Miete, Quad_m, KT, Ort)
data_subset
```


### 4. Aim: Where are the biggest apartments

```{r, top10 biggest apartments}
...
### 5.Aim: Show relationship of the population in the canton and size of apartments
...
summary(simple.fit)
```
The above applied simple regression analysis shows that the rental price varies depending on the apartment area. 
...
summary(multi.fit)
```
The above multi-regression analysis shows that the price of an apartment is dependent on many factors such as apartment area, population & commune area, but not on the number of rooms.
...
The residuals analysis above shows that the plot residuals vs. fitted values can be presented in a linear model. Therefore we consider that the chosen "lm" model presents the dataset well.
...
plot(multi.fit)
```
The above residual analysis for the multi regression analysis shows also that the residuals and the fitted values can be modelled with a linear model well.
...
Selecting out data one can test different plots.
